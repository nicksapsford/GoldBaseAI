...
def _reconcile_and_adopt(stanley, where):
    ...
    and tested (19-20 Aug). Fires ONE adoption/abort notification. Idempotent + safe: genuinely flat, or
    paper mode, -> harmless no-op. Never raises."""
    try:
...
def _sync_account(ig, stanley, account=None):
    ...
    try:
        ...
        if ig.set_account(desired):
            ...
            # left flat-on-live (the duplicate-entry gap: Rule 13 forces DEMO on every reboot, so the manual
            # flip is where a pre-existing live position first becomes visible). Only on the ->LIVE direction.
