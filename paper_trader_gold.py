...
def _real_close_fill(ig, epic, trade, tries=3, delay=2.0):
    ...
    camelot_engine.live_executor.external_close_price() -- the real-P&L-derived exit already proven for
    EXTERNAL_CLOSE. The closing txn appears a moment after the close order, so retry briefly. Returns a float
    exit price, or None -> the caller falls back to the feed estimate + alerts. Read-only; never raises."""
    try:
        import time as _t
...
class PaperTraderGold:
    ...
    def close_trade(self, price: float, reason: str, gbpusd: float = None) -> Optional[GoldTrade]:
        ...
        if LIVE_EXECUTION and self.ig is not None and deal_id:
            ...
            # was understating ~£12 on the first force-close since that fix. Covers every engine-initiated market
            # close. EXCLUDES EXTERNAL_CLOSE (the caller already passes the real fill) and STOP_LOSS (keeps the
            # deliberate Job-10 stop-level clamp above). Falls back to the feed estimate + alerts if not visible.
