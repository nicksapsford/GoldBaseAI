"""
GBPUSD BACKTEST -- strategy_gbpusd.py (compass Session 2)
GBPUSD (C:GBPUSD) mechanics. COPY of strategy_gold, pip-scaled (1 pip = 0.0001; price ~1.3534).
Backtest-only; NOT a live system.
...
TRAILING_STOP_POINTS   = 0.0030    # trailing stop in gold points ($/oz). GoldBase v1.0.2 (6 Aug 2026):
...
TAKE_PROFIT_POINTS     = 0.0020    # Commission 025 exit fix (14 Aug 2026): banks the common move before the trail gives it back    # scalp target. GoldBase v1.0.2: raised 50->90 (Commission 019) -- the
...
TIGHT_TRAIL_ACTIVATE_POINTS = 0.00225   # profit (pt) at which the trail tightens 40 -> 10
TIGHT_TRAIL_POINTS          = 0.00075   # tight trailing distance (pt) after activation
...
SPREAD_POINTS          = 0.0001    # Capital.com gold spread (very low cost)
...
def calculate_pnl(entry_price: float, exit_price: float, direction: str,
    ...
    return round(points, 6), round(pnl_usd, 2), round(pnl_gbp, 2)
...
class GoldTrade:
    ...
    def apply_profit_ladder(self, price: float):
        ...
        if self.direction == "LONG":
            floor_stop = round(self.entry_price + floor_pts, 6)
    ...
    def mfe_pts(self) -> float:
        return round(getattr(self, "_mfe_pts", 0.0), 6)

    @property
    def mae_pts(self) -> float:
        return round(getattr(self, "_mae_pts", 0.0), 6)
